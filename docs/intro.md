...
后续的实验文档会指引同学们去阅读相应章节，但不会复述标准中的内容，避免产生歧义、过时等情况造成误导。我们会在每个文档指引下放置一个折叠的「要点」，强烈建议同学们在**自行阅读文档后**再对照总结进行自测。如果你实在不愿意、没时间或看不懂文档，可以直接看总结。**但遇到不明白的点或者产生问题时，还是要回去看文档。**

下面是实验文档的格式介绍：

!!! question "考点"

    实验文档中标记为考点的方块是希望同学们能掌握的知识点，会在验收时提问。
